...
The plugin launcher sets `https://www.coohom.com/pub/tool/bim/ai-home/mcp-executor` as the production base for task URLs through the public `LUX3D_MCP_EXECUTOR_URL` option when no non-empty override is configured. It is not a direct plugin launch URL. Other environments use the public MCP environment variable `LUX3D_MCP_EXECUTOR_URL`, restricted to HTTPS on `coohom.com` or its subdomains; it is not a tool argument. If the requested environment differs from the returned URL, explain the mismatch and confirm configuration changes rather than rewriting the URL.
