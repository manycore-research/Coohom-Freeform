...
For interior lighting discover the current public get_lights/set_lights capabilities and read their complete descriptions, input schemas and output schemas when available, plus public help. Record full-replacement semantics, supported types, units, stable-ID rules, empty-list effects, render readiness and synchronization-result semantics. Missing lighting tools mean functional lighting is unsupported in this session; explain the limitation without private APIs, decorative geometry or silent MCP upgrades.

Read the actual response envelope before extracting lights or render state. When no outputSchema exists, use public descriptions and real returned data, without freezing a host-source path into the skill. Undocumented observations require explanation and user confirmation before reliance, not invented type extensions. Distinguish active rendering with a ready channel from a merely accepted synchronization request; neither proves a visible new image. Follow [lighting.md](lighting.md) for entry, full-list preservation and rendered acceptance. Refresh this record after a reload and keep configuration verification separate from visual evidence.

## Asset handoff

Before paid generation, check that at least one route declared by `freeform-modeling-mcp` can consume the kinds of output declared by `@manycore/coohom-lux3d-mcp`. This is a preflight, not proof of future generation/import success. If no route exists, skip generation and follow [Automatic technical fallback](ai-generation.md#automatic-technical-fallback) when Freeform is ready; do not ask the user to solve technical contract matching.
...
- If several valid routes have equivalent effect, prefer the one retaining asset identity. During technical fallback, choose ordinary approximations automatically within the original requirements; ask only if a route would change explicit design constraints or needs additional user authorization.
- If neither route is possible, record the exact missing requirement, preserve the generated output and follow automatic technical fallback. After a failed route, at most one alternative may be attempted with complete declared inputs and a reconciled scene/job state; no route cycling. Do not fabricate dimensions, add undeclared parameters, regenerate, or change versions automatically.
...
Installation and explicit updates request both MCPs at `latest` by default. For a fresh ZIP installation with a user-selected exact pair, use `Install.cmd --install-mcp-versions <freeform-exact> <lux3d-exact>` (macOS: `./Install.command`). This does not bypass failed-install recovery or change the default dependency policy. Ordinary startup uses the installed pair; changing plugin prose does not update dependencies. A running task keeps its processes and records until the user chooses an appropriate restart. Do not upgrade MCPs mid-generation or during scene writes.
...
- Installation succeeded but initialization, schema semantics or asset handoff failed: record it as a distinct compatibility/readiness issue. During scene work use the eligible technical asset fallback when Freeform is ready; during maintenance report the compatibility limitation. Do not reclassify it as an installation failure or silently change dependency versions.
...
Marketplace: use `scripts/bootstrap freeform status|install|retry|versions [freeform-exact lux3d-exact]` (Windows: `scripts/bootstrap.cmd`). This targets the shared pair for both services, not just `freeform-modeling-mcp`. Preserve the reported cache root. Inspect status before acting; status is read-only even when Node is missing. For the unified upgrade and cleanup workflow, follow [maintenance.md](maintenance.md). Never delete the recovery state to bypass the decision flow. Interrupted locks require process/state diagnosis before removing a confirmed stale lock.
