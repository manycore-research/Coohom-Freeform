...
- Freeform is temporarily pinned to public npm `freeform-modeling-mcp@1.0.34`: the newer `latest` CLI entry is incompatible with the current adapter. Lux3D continues to use `@manycore/coohom-lux3d-mcp@latest`. Successful installations record exact versions and lockfiles; later starts reuse them. Freeform uses `npm ci` with its complete dependency tree frozen in the shipped `bundler/freeform-package-lock.json`; Lux3D remains independent.
